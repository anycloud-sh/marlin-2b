...
def run(args):
    image = args.image
    if not re.fullmatch(r"ghcr.io/anycloud-sh/marlin-2b@sha256:[0-9a-f]{64}", image):
        raise ValueError("An exact Marlin candidate digest is required")
    ...
    record()
    try:
        input_bucket = "anycloud-marlin-input-" + suffix
